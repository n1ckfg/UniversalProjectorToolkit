...
void setup() { 
  ...
  host = new Host(hostWidth, hostHeight, "kinect1");
  
  // set up kinect 
  opencv = new OpenCV(this, hostWidth, hostHeight);
...
void draw() {
  ...
  depthMap = host.depthMap;
  opencv.loadImage(host.rgb.get());
...
void drawGui() {
  ...
  pushMatrix();
  pushStyle();
  translate(host.depth.width+70, 40); // this is black box
  fill(0);
...
PVector getDepthMapAt(int x, int y) {
  PVector dm = depthMap[host.depth.width * y + x];
